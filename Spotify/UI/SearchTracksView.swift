import SwiftUI

struct SearchTracksView: View {
    @Binding var searchText: String
    let selectedPlaylistID: String
    @StateObject private var viewModel = SearchTracksViewModel()

    var body: some View {
        List {
            ForEach(viewModel.searchResults) { track in
                HStack {
                    AsyncImage(url: URL(string: track.cover_medium ?? "")) { image in
                        image.resizable().scaledToFill()
                    } placeholder: {
                        Color.gray.opacity(0.3)
                    }
                    .frame(width: 50, height: 50)
                    .clipShape(RoundedRectangle(cornerRadius: 8))

                    VStack(alignment: .leading) {
                        Text(track.title)
                            .font(.headline)
                        Text(track.artistName)
                            .font(.caption)
                            .foregroundColor(.gray)
                    }

                    Spacer()

                    Button {
                        viewModel.playOrPause(track: track)
                    } label: {
                        Image(systemName: viewModel.currentlyPlayingTrackID == track.id ? "pause.circle.fill" : "play.circle.fill")
                            .resizable()
                            .frame(width: 24, height: 24)
                            .foregroundColor(.green)
                    }
                    .buttonStyle(.plain)
                    .padding(.trailing, 8)

                    Button {
                        viewModel.toggleTrackInPlaylist(track, playlistID: selectedPlaylistID)
                    } label: {
                        Image(systemName: "plus.circle.fill")
                            .resizable()
                            .frame(width: 24, height: 24)
                            .foregroundColor(viewModel.addedTrackIDs.contains(track.id) ? .green : .gray)
                    }
                    .buttonStyle(.plain)
                }
                .padding(.vertical, 4)
            }
        }
        .onChange(of: searchText) {
            Task { await viewModel.searchTracks(with: searchText) }
        }
        .onAppear {
            Task { await viewModel.fetchPlaylistTrackIDs(for: selectedPlaylistID) }
        }
    }
}
